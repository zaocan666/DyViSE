...
from models.ECAPA_lip.lip_model.resnet import ResNet, BasicBlock
from models.ECAPA_lip.lip_model.shuffle import ShuffleNetV2
from models.ECAPA_lip.lip_model.tcn import MultibranchTemporalConvNet, TemporalConvNet
...
class TCN(nn.Module):
    ...
    def forward(self, x, B, extract_feats=False):
        # x needs to have dimension (N, C, L) in order to be passed into CNN
        if extract_feats:
            x = self.tcn_trunk(x.transpose(1, 2)) # (N, C, L)
            return x.unsqueeze(-2) # (N, C, 1, L)
        else:
...
class Lipreading(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, Tnew, x.size(1)) # (B, T, C)
        #return x if self.extract_feats else self.tcn(x, lengths, B)
        # return self.tcn(x, lengths, B, self.extract_feats)
        return self.tcn(x, B, self.extract_feats)


def get_lip_reading_model(checkpoint_pth):
    args_loaded = {
    ...
    net.load_state_dict(torch.load(checkpoint_pth, map_location=lambda storage, loc: storage))
    return net